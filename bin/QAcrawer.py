...
TotalPageNum = 1
# KeywordFile = 'key_word_7000'
# KeywordFile = 'key_word2'
# KeywordFile = '样例数据.txt'
KeywordFile = 'questions.txt'
...
def craw(url_num, page_num, file):
	if url_num == 0:
		# url = urls[0] + urllib.quote(question_word.split('\t')[0].decode(sys.stdin.encoding).encode('gbk')) + "&pn="
		url = urls[0] + urllib.quote(question_word.decode(sys.stdin.encoding).encode('gbk')) + "&pn="
		...
		# url = urls[1] + urllib.quote(question_word.split('\t')[0].decode(sys.stdin.encoding).encode('gbk')) + "&pn="
