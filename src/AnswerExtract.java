...
{
    private final String file_type = "200";//200 or 7000;

    private final String OpenClose = "open";//open or close;

    private boolean check_answer = true;//200 could check answer, 7000 could not;

    //close 7000
//    private final String file_snippet_prefix = "final_7000/v3_7000_";

    //open 7000 & 200
    private final String file_snippet_prefix = "snippet_zhidao_" + file_type + "/snippet_for_";
    private final String file_key_words = "in/key_word_" + file_type;
    private final String file_answer_type = "in/answer_type_" + file_type;
    private final String file_test = "in/test.txt";


    private final String file_question = "in/question_" + file_type + ".txt";
    private final String file_vectors = "in/vectors.bin." + file_type;

    private final String file_predict_sentence = "meta/predict_sentence.txt";
    private final String file_predict_words = "meta/predict_words.txt";

    private final String file_res = "out/results" + file_type + ".txt";
    ...
    public AnswerExtract() throws IOException
    {
        ...
        StopWord = new ArrayList<String>(Arrays.asList("��", "��", "��", "��", "��", "�п���", "��", "��", "Ϊ"));
    ...
    public void sentence_words_extract() throws IOException
    {
        ...
        while((question = br_key_word.readLine()) != null)
        {
            ...
            final HashMap<String, Integer> sort_sentence = new HashMap<String, Integer>();
            for(int i = 0; i < cadidate_Sentences.size(); i++)
            {
...
//            ArrayList<String> a = get_keywords(question);
            ...
            String myanswer = template_get_answer(question, a, sentence, b, answertype, ques_cnt);
                        if(myanswer != null)
            {
...
//            System.out.print("\n");
            ...
            final HashMap<String, Double> sort_answer = new HashMap<String, Double>();
            for(int i = 0; i < b.size(); i++)
            {
...
//            System.out.println(termList);
                ...
                final HashMap<String, Double> sort_answer_excep = new HashMap<String, Double>();
...
//                cnt_ideal++;
    ...
    public String template_get_answer(String question, ArrayList<String> key_words,
            String sentence, ArrayList<String> candidate_words, String answertype, int ques_cnt) throws IOException
    {    	
     	String answerFromZhidao = askZhidao(ques_cnt);
    	if(answerFromZhidao!=null)
    		return answerFromZhidao;
    	
       	// 年份的模板, remove unrelated numbers
    	if(answertype.contains("m")){
//    		//System.out.println(candidate_words.size());
//    		//System.out.println(offsetList.size());
        	// “第几”的模板, remove unrelated numbers
        	if(question.contains("是第几") || question.contains("第几大")){
        		Iterator<String> betterword_it = candidate_words.iterator();
        		int index = 0, deletecnt = 0;
                while(betterword_it.hasNext()){
                    String betterword = betterword_it.next();
                    if(!betterword.contains("第") || betterword.length() == 1){
                    	betterword_it.remove();
                    	deletecnt++;
                    }
                    else{
                    	candidate_words.set(index-deletecnt, betterword);
                    }
                    index++;
                }
//                //System.out.println(candidate_words.size());
//        		//System.out.println(offsetList.size());
        	}else/* if(question.contains("第几")){
               	//System.out.println(question);
            	//System.out.println(sentence);
            	//System.out.println(answertype);
              	//System.out.println(candidate_words);
       

                Pattern p = Pattern.compile( "第几.");
                Matcher m = p.matcher(question);
                String maskkeyword = m.group().substring(2, 3);
                
                p = Pattern.compile( "第."+maskkeyword);
                ...
                if(m.find())
                {
                	...
                	return m.group().substring(0, 2);       			
                }
        	}
        	

        	else*/ if(question.contains("哪一年") || question.contains("哪年") || question.contains("年份是")){
	    		Iterator<String> betterword_it = candidate_words.iterator();
        		int index = 0, deletecnt = 0;	    		
                while(betterword_it.hasNext()){
                    String betterword = betterword_it.next();
	                if(betterword.length() == 4 && sentence.regionMatches(sentence.indexOf(betterword), betterword+"年", 0, 5)){
	                	candidate_words.set(index-deletecnt, betterword);
	                }
	                else{
	                	betterword_it.remove();
...
//                    //System.out.println(m.group(0));
    ...
    private String askZhidao(int ques_num) throws IOException {
 		String zhidaoBaseFile = file_snippet_prefix + ques_num + ".txt";
